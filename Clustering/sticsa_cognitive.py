...
data=pd.read_csv('/Users/xinxing/Desktop/MDD/New data/MDD_test/MDD_score/MDD_sticsa_cognitive.csv').fillna(0)
...
df=data[["Age","sticsa_cognitive"]] 
df=df.reset_index(drop=True)

'''
data=data[['Status',"Linsula", "Rprecentral","N_CSF","Rparacentral",
           "LGsuperiortemporal","Lparstriangularis","Lbankssts",
           "N_3rd-Ventricle","RGcuneus","Rtransversetemporal"]] 

index=["Linsula", "Rprecentral","N_CSF","Rparacentral",
           "LGsuperiortemporal","Lparstriangularis","Lbankssts",
           "N_3rd-Ventricle","RGcuneus","Rtransversetemporal"]
'''
...
hierarchy.dendrogram(Z, color_threshold=11, above_threshold_color='grey')
 
# Add horizontal line.
plt.axhline(y=11, c='black', lw=2, linestyle='dashed')
...
print(("Group 0 avg. score is: ") + str(df_concat[df_concat.label==0]["sticsa_cognitive"].mean()))

print(("Group 1 avg. score is: ") + str(df_concat[df_concat.label==1]["sticsa_cognitive"].mean()))
from scipy.stats import ttest_ind
stat, p = ttest_ind(df_concat[df_concat.label==0].Age,df_concat[df_concat.label==1].Age)

print("p-value is: "+str(p))

stat_score, p_score = ttest_ind(df_concat[df_concat.label==0].sticsa_cognitive,df_concat[df_concat.label==1].sticsa_cognitive)
