...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 22 14:24:17 2020
...
data=pd.read_csv('/Users/xinxing/Desktop/MDD/New data/MDD_test/MDD_score/MDD_qids_score.csv').fillna(0)


'''
#MDD+CN:
data=data[['Status',"N_Right-Inf-Lat-Vent",""]] 
'''
#MDD only:
data=data[data.Status==0]
df=data[["Age","qids_score"]] 
df=df.reset_index(drop=True)
'''
data=data[['Status',"N_Right-Inf-Lat-Vent","Rprecuneus","Rcuneus","Rparsopercularis",
           "N_Left-Lateral-Ventricle","Rparstriangularis"]] 

index=["N_Right-Inf-Lat-Vent","Rprecuneus","Rcuneus","Rparsopercularis",
           "N_Left-Lateral-Ventricle","Rparstriangularis"]
'''
data=data[['Status',"Rprecuneus","Rcuneus"]]
index=["Rprecuneus","Rcuneus"]
...
hierarchy.dendrogram(Z, color_threshold=6.3, above_threshold_color='grey')
 
# Add horizontal line.
plt.axhline(y=6.3, c='black', lw=2, linestyle='dashed')
...
print(("Group 0 avg. score is: ") + str(df_concat[df_concat.label==0]["qids_score"].mean()))

print(("Group 1 avg. score is: ") + str(df_concat[df_concat.label==1]["qids_score"].mean()))
...
stat_score, p_score = ttest_ind(df_concat[df_concat.label==0].qids_score,df_concat[df_concat.label==1].qids_score)
