...
# -*- coding: utf-8 -*-
"""
Created on Sun Nov 22 14:47:47 2020
...
data=pd.read_csv('/Users/xinxing/Desktop/MDD/New data/MDD_test/MDD_score/MDD_shaps_score_1.csv').fillna(0)


'''
#MDD+CN:
data=data[['Status',"Rprecentral", "Llateralorbitofrontal"]] 
'''
#MDD only:
data=data[data.Status==0]
df=data[["Age","shaps_score_1"]] 
df=df.reset_index(drop=True)

'''
data=data[['Status',"Rprecentral","Rprecuneus","RGcaudalmiddlefrontal",
           "N_Left-Inf-Lat-Vent","LGcaudalanteriorcingulate","N_Left-Thalamus-Proper",
           "Risthmuscingulate"]]
 
index=["Rprecentral","Rprecuneus","RGcaudalmiddlefrontal",
           "N_Left-Inf-Lat-Vent","LGcaudalanteriorcingulate","N_Left-Thalamus-Proper",
           "Risthmuscingulate"]
'''
data=data[['Status',"Rprecentral","Rprecuneus","LGcaudalanteriorcingulate","N_Left-Thalamus-Proper"]]
index=["Rprecentral","Rprecuneus","LGcaudalanteriorcingulate","N_Left-Thalamus-Proper"]
...
print(("Group 0 avg. score is: ") + str(df_concat[df_concat.label==0]["shaps_score_1"].mean()))

print(("Group 1 avg. score is: ") + str(df_concat[df_concat.label==1]["shaps_score_1"].mean()))
from scipy.stats import ttest_ind
stat, p = ttest_ind(df_concat[df_concat.label==0].Age,df_concat[df_concat.label==1].Age)

print("p-value is: "+str(p))

stat_score, p_score = ttest_ind(df_concat[df_concat.label==0].shaps_score_1,df_concat[df_concat.label==1].shaps_score_1)
